...
def main():
	...
	url = 'https://moto.data.socrata.com/resource/f42u-v6f3.json'#url to retrieve victoria dataset from Socrata

	urllib.request.urlretrieve(url, "victoria.json")#make a request to fetch the required resource


	#Deleting previous victoria.json datasets
	hdfs_path = "/user/klnu/victoria.json"
	...
	file_path = "/home/klnu/victoria.json"
	hdfs_path = os.path.join(os.sep, 'user', 'klnu', "victoria.json")
	...
	crime_data = spark.read.json("victoria.json", multiLine=True)
	...
	#Appending the base_table(table where crime data for other cities are stored) with victoria dataframe
